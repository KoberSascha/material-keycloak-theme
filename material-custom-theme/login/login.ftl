...
<@layout.registrationLayout; section>
                    ...
                    <div id="kc-form-options" class="${properties.kcFormOptionsClass!}">
                        ...
                        <div id="kc-form-buttons" class="col-xs-5 ${properties.kcFormButtonsClass!}">
                            <div class="${properties.kcFormButtonsWrapperClass!}">
                                <button class="mdc-button mat-primary mdc-button--raised ${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!} ${properties.kcButtonLargeClass!}" name="login" id="kc-login" type="submit">
                                    ${msg("doLogIn")}
                                </button>
                            </div>
                    ...
                    <#if (realm.password && realm.registrationAllowed && !usernameEditDisabled??) || realm.resetPasswordAllowed>
                        ...
                        <div class="d-flex-center">
                            <#if realm.password && realm.registrationAllowed && !usernameEditDisabled??>
                                <div id="kc-registration" class="col-xs-12 text-center" <#if realm.password && realm.registrationAllowed && !usernameEditDisabled?? && realm.resetPasswordAllowed>style="margin-bottom: 15px;"</#if>>
                                    <span>${msg("noAccount")} <a class="mdc-button  mdx-button--raised" href="${url.registrationUrl}">${msg("doRegister")}</a></span>
                                </div>
                            </#if>
                        </div>
                        <div class="d-flex-center">
                            <div class="${properties.kcFormOptionsWrapperClass!} col-xs-12 text-center">
                                <#if realm.resetPasswordAllowed>
                                    <span><a class="mdc-button  mdx-button--raised" href="${url.loginResetCredentialsUrl}">${msg("doForgotPassword")}</a></span>
                                </#if>
                            </div>
                        </div>
                    </#if>
    ...
    <#elseif section = "info" >
        <#if realm.password && social.providers??>
            <div id="kc-social-providers text-center">
                <ul>
